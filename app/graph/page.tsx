...
export default function GraphPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 pt-32 pb-8 px-4 sm:px-6 flex flex-col">
      <div className="max-w-7xl mx-auto w-full flex-grow flex flex-col space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <h1 className="text-3xl font-bold text-gray-900">Case Linkage Topology</h1>
          ...
          <div className="flex items-center gap-3">
            <button onClick={fetchGraphData} className="px-4 py-2 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition flex items-center gap-2 text-sm text-gray-700 shadow-sm">
        ...
        {error && (
          <div className="w-full bg-red-500/10 border border-red-500/30 p-4 rounded-xl flex items-center gap-3 text-red-600">
            <AlertCircle size={20} />
            <p>{error}</p>
          </div>
        )}
        ...
        {/* Legend */}
        <div className="flex items-center gap-6 bg-white border border-gray-200 px-6 py-3 rounded-2xl w-max backdrop-blur-sm shadow-sm">
          <div className="flex items-center gap-2 text-sm text-gray-700">
            <span className="w-3 h-3 rounded-full bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.5)]"></span> Evaluated Article
          </div>
          <div className="flex items-center gap-2 text-sm text-gray-700">
            <span className="w-3 h-3 rounded-full bg-orange-500 shadow-[0_0_10px_rgba(249,115,22,0.5)]"></span> Cited Precedent
          </div>
        </div>
        ...
        <div
          ref={containerRef}
          className="w-full flex-grow min-h-[500px] lg:min-h-[600px] border border-gray-200 rounded-3xl overflow-hidden relative bg-white group cursor-grab active:cursor-grabbing shadow-inner"
        >
          {loading ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/40 backdrop-blur-sm z-10">
              <Network size={48} className="text-indigo-600 animate-pulse mb-4 opacity-50" />
              <p className="text-indigo-700 font-medium tracking-widest text-sm">CALCULATING PHYSICS...</p>
            </div>
          ) : (
            <ForceGraph2D
